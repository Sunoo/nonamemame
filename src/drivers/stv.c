...
static WRITE32_HANDLER( minit_w )
{
	...
	// quick hack just to make sure the slave is always ready for a command ... probably won't work with the games
	// as it will kill the master cpu performance but might be ok while we attempt to get around our boot problems
	cpu_spinuntil_time(TIME_IN_USEC(400000));
