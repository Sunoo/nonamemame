...
static READ16_HANDLER( archrivl_port_1_r )
{
	...
	/*    01xx  (4 5 6 7)  = full left/up                         */
	/*    001x  (2 3)      = middle left/up                       */
...
/*start MAME:analog+*/
	...
	/* the next pair of ifs (3rd & 4th) handle digital (8-way) input         */
	/* the fifth if handles a just centered digital input for that axis      */
	/* the final else handles input from analog devices                      */
	/* this setup gives precidence to 49-way input, then 8-way, then analog  */
	if (result & 0x00f0)	        { truestickmem = (truestickmem & ~0x00f0) | (result & 0x00f0); }
	else if (truestickmem & 0x00f0) { truestickmem &= ~0x00f0; }
	else if (dstick & 0x0001)       { result |= 0x0040; dstickmem |= 0x0001; }
	else if (dstick & 0x0002)       { result |= 0x0090; dstickmem |= 0x0002; }
	else if (dstickmem & 0x0003)    { result |= (0x0f << 4); dstickmem &= ~0x0003; }
	else
	{
		joy_y = readinputport(5) >> 4;	/* 0 = down 3 = center 6 = up */
		bits_y = (0x70 >> (7 - joy_y)) & 0x0f;
		result |= (~bits_y &0x0f) << 4;
	}

	if (result & 0x000f)	        { truestickmem = (truestickmem & ~0x000f) | (result & 0x000f); }
	else if (truestickmem & 0x000f) { truestickmem &= ~0x000f; }
	else if (dstick & 0x0004)       { result |= 0x0004; dstickmem |= 0x0004; }
	else if (dstick & 0x0008)       { result |= 0x0009; dstickmem |= 0x0008; }
	else if (dstickmem & 0x000C)    { result |= 0x0f; dstickmem &= ~0x000C; }
	else
	{
	...
	else if (dstick & 0x0010)       { result |= 0x4000; dstickmem |= 0x0010; }
	...
	else
	{
		...
		bits_y = (0x70 >> (7 - joy_y)) & 0x0f;
		result |= (~bits_y &0x0f) << 12;
	}

	if (result & 0x0f00)	        { truestickmem = (truestickmem & ~0x0f00) | (result & 0x0f00); }
	else if (truestickmem & 0x0f00) { truestickmem &= ~0x0f00; }
	else if (dstick & 0x0040)       { result |= 0x0400; dstickmem |= 0x0040; }
...
/*start MAME:analog+*/
	...
	else if (dstick & 0x0001)       { result |= 0x4000;  dstickmem |= 0x0001; }
	...
	else if (dstick & 0x0004)       { result |= 0x0400; dstickmem |= 0x0004; }
...
/*start MAME:analog+*/
	...
	if (result & 0x0f00)	        { truestickmem |= (result & 0x0f00); }
	else if (truestickmem & 0x0f00) { truestickmem &= ~0x0f00; }
	else if (dstick & 0x0040)       { result |= 0x0400; dstickmem |= 0x0040; }
	else if (dstick & 0x0080)       { result |= 0x0900; dstickmem |= 0x0080; }
	else if (dstickmem & 0x00C0)    { result |= (0x0f << 8); dstickmem &= ~0x00C0; }
...
/*Analog+ start */
	PORT_BIT( 0x0001, IP_ACTIVE_HIGH, IPT_BUTTON1 ) PORT_PLAYER(5)  /* P1 left-right (A1) opto 1 (joy pin 2?) */
	PORT_BIT( 0x0002, IP_ACTIVE_HIGH, IPT_BUTTON2 ) PORT_PLAYER(5)  /* P1 left-right (B1) opto 2 (joy pin 3?) */
	PORT_BIT( 0x0004, IP_ACTIVE_HIGH, IPT_BUTTON3 ) PORT_PLAYER(5)  /* P1 left-right (C1) opto 3 (joy pin 4?) */
	PORT_BIT( 0x0008, IP_ACTIVE_HIGH, IPT_BUTTON4 ) PORT_PLAYER(5)  /* P1 left-right direction bit (joy pin 5?) */
	PORT_BIT( 0x0010, IP_ACTIVE_HIGH, IPT_BUTTON5 ) PORT_PLAYER(5)  /* P1  up-down   (A1) opto 4 (joy pin 6?) */
	PORT_BIT( 0x0020, IP_ACTIVE_HIGH, IPT_BUTTON6 ) PORT_PLAYER(5)  /* P1  up-down   (B1) opto 5 (joy pin 7?) */
	PORT_BIT( 0x0040, IP_ACTIVE_HIGH, IPT_BUTTON7 ) PORT_PLAYER(5)  /* P1  up-down   (C1) opto 6 (joy pin 8?) */
	PORT_BIT( 0x0080, IP_ACTIVE_HIGH, IPT_BUTTON8 ) PORT_PLAYER(5)  /* P1  up-down   direction bit (joy pin 9?) */
	PORT_BIT( 0x0100, IP_ACTIVE_HIGH, IPT_BUTTON1 ) PORT_PLAYER(6)  /* P2 left-right (A1) opto 1 (joy pin 2?) */
	PORT_BIT( 0x0200, IP_ACTIVE_HIGH, IPT_BUTTON2 ) PORT_PLAYER(6)  /* P2 left-right (B1) opto 2 (joy pin 3?) */
	PORT_BIT( 0x0400, IP_ACTIVE_HIGH, IPT_BUTTON3 ) PORT_PLAYER(6)  /* P2 left-right (C1) opto 3 (joy pin 4?) */
	PORT_BIT( 0x0800, IP_ACTIVE_HIGH, IPT_BUTTON4 ) PORT_PLAYER(6)  /* P2 left-right direction bit (joy pin 5?) */
	PORT_BIT( 0x1000, IP_ACTIVE_HIGH, IPT_BUTTON5 ) PORT_PLAYER(6)  /* P2  up-down   (A1) opto 4 (joy pin 6?) */
	PORT_BIT( 0x2000, IP_ACTIVE_HIGH, IPT_BUTTON6 ) PORT_PLAYER(6)  /* P2  up-down   (B1) opto 5 (joy pin 7?) */
	PORT_BIT( 0x4000, IP_ACTIVE_HIGH, IPT_BUTTON7 ) PORT_PLAYER(6)  /* P2  up-down   (C1) opto 6 (joy pin 8?) */
	PORT_BIT( 0x8000, IP_ACTIVE_HIGH, IPT_BUTTON8 ) PORT_PLAYER(6)  /* P2  up-down   direction bit (joy pin 9?) */
...
/*Analog+ start */	/* original 49 way player 1 joystick goes here */
	PORT_BIT( 0x0100, IP_ACTIVE_HIGH, IPT_BUTTON1 ) PORT_PLAYER(5)  /* left-right (A1) opto 1 (joy pin 2?) */
	PORT_BIT( 0x0200, IP_ACTIVE_HIGH, IPT_BUTTON2 ) PORT_PLAYER(5)  /* left-right (B1) opto 2 (joy pin 3?) */
	PORT_BIT( 0x0400, IP_ACTIVE_HIGH, IPT_BUTTON3 ) PORT_PLAYER(5)  /* left-right (C1) opto 3 (joy pin 4?) */
	PORT_BIT( 0x0800, IP_ACTIVE_HIGH, IPT_BUTTON4 ) PORT_PLAYER(5)  /* left-right direction bit (joy pin 5?) */
	PORT_BIT( 0x1000, IP_ACTIVE_HIGH, IPT_BUTTON5 ) PORT_PLAYER(5)  /*  up-down   (A1) opto 4 (joy pin 6?) */
	PORT_BIT( 0x2000, IP_ACTIVE_HIGH, IPT_BUTTON6 ) PORT_PLAYER(5)  /*  up-down   (B1) opto 5 (joy pin 7?) */
	PORT_BIT( 0x4000, IP_ACTIVE_HIGH, IPT_BUTTON7 ) PORT_PLAYER(5)  /*  up-down   (C1) opto 6 (joy pin 8?) */
	PORT_BIT( 0x8000, IP_ACTIVE_HIGH, IPT_BUTTON8 ) PORT_PLAYER(5)  /*  up-down   direction bit (joy pin 9?) */
...
/*Analog+ start */	/* real 49 way player 2 joystick goes here */
	PORT_BIT( 0x0100, IP_ACTIVE_HIGH, IPT_BUTTON1 ) PORT_PLAYER(6)  /* left-right (A1) opto 1 (joy pin 2?) */
	PORT_BIT( 0x0200, IP_ACTIVE_HIGH, IPT_BUTTON2 ) PORT_PLAYER(6)  /* left-right (B1) opto 2 (joy pin 3?) */
	PORT_BIT( 0x0400, IP_ACTIVE_HIGH, IPT_BUTTON3 ) PORT_PLAYER(6)  /* left-right (C1) opto 3 (joy pin 4?) */
	PORT_BIT( 0x0800, IP_ACTIVE_HIGH, IPT_BUTTON4 ) PORT_PLAYER(6)  /* left-right direction bit (joy pin 5?) */
	PORT_BIT( 0x1000, IP_ACTIVE_HIGH, IPT_BUTTON5 ) PORT_PLAYER(6)  /*  up-down   (A1) opto 4 (joy pin 6?) */
	PORT_BIT( 0x2000, IP_ACTIVE_HIGH, IPT_BUTTON6 ) PORT_PLAYER(6)  /*  up-down   (B1) opto 5 (joy pin 7?) */
	PORT_BIT( 0x4000, IP_ACTIVE_HIGH, IPT_BUTTON7 ) PORT_PLAYER(6)  /*  up-down   (C1) opto 6 (joy pin 8?) */
	PORT_BIT( 0x8000, IP_ACTIVE_HIGH, IPT_BUTTON8 ) PORT_PLAYER(6)  /*  up-down   direction bit (joy pin 9?) */
//	PORT_BIT( 0xff00, IP_ACTIVE_HIGH, IPT_UNUSED )	/* original mame code */
/*Analog+ end*/
