...
#if (DEFAULT_PWD_ENABLED == TRUE)
	#define DEFAULT_PASSWORD "*" DEFAULT_PWD_VALUE
#else
	#define DEFAULT_PASSWORD " " DEFAULT_PWD_VALUE
#endif
...
#else
	{ "default_game",               RO_STRING,  &settings.default_game,              "pong" },
#endif
...
#ifdef OUTLAW
	{ "show_unavailable",           RO_INT,     &settings.show_unavailable,          "1" },
	{ "show_recent",                RO_INT,     &settings.show_recent,               "1" },
#else
...
static REG_OPTION regGameOpts[] =
{
	...
	// Analog+
	{ "multiplemice",           RO_BOOL,    &gOpts.multimouse,                      "0" },
	{ "switchablemice",         RO_BOOL,    &gOpts.switchmice,                      "0" },
	{ "switchmiceaxes",         RO_BOOL,    &gOpts.switchaxes,                      "0" },
	{ "splitmouseaxes",         RO_BOOL,    &gOpts.splitmouse,                      "0" },
	{ "resetmouseaxes",         RO_BOOL,    &gOpts.resetmouse,                      "0" },
	{ "lightgun2a",             RO_BOOL,    &gOpts.use_lightgun2a,                  "0" },
	{ "lightgun2b",             RO_BOOL,    &gOpts.use_lightgun2b,                  "0" },
	{ "williams49way",          RO_BOOL,    &gOpts.use_williams49way,               "0" },
...
BOOL GetPassword(char *pwd)
{
	...
	return (*settings.password=='*' ? TRUE : FALSE);
}
...
void PasswordDecodeString(const char *str, void *data)
{
	...
	*(char **)data = (char *)malloc(strlen(str)/3);
	...
	// wrong checksum, password set to its default state and value
	if ( car != chk )
	{
		strcpy(p, DEFAULT_PASSWORD);
	}
}
