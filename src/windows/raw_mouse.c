...
BOOL init_raw_mouse(BOOL in_include_sys_mouse, BOOL in_include_rdp_mouse, BOOL in_include_individual_mice)
{
	...
	for (i = 0; i < nInputDevices; i++) {
		if (pRawInputDeviceList[i].dwType == RIM_TYPEMOUSE) {
			...
			if ((psName = malloc(sizeof(TCHAR) * nSize)) == NULL)  {
	...
	if ((raw_mice = malloc(sizeof(RAW_MOUSE) * raw_mouse_count)) == NULL)  {
	...
	raw_mouse_count -= excluded_sysmouse_devices_count;
...
BOOL is_raw_mouse_button_pressed(int mousenum, int buttonnum) {
	...
	if (mousenum >= raw_mouse_count || buttonnum >= MAX_RAW_MOUSE_BUTTONS || raw_mice == NULL) return 0;
...
char *get_raw_mouse_button_name(int mousenum, int buttonnum) {
	if (mousenum >= raw_mouse_count || buttonnum >= MAX_RAW_MOUSE_BUTTONS || raw_mice == NULL) return NULL;
...
ULONG get_raw_mouse_x_delta(int mousenum)
{
	...
	if (raw_mice != NULL && mousenum < raw_mouse_count) {
