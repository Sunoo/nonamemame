	...
	*buffer = 0;
	...
	for (i = 1; i < CPU_COUNT; i++)
	{
		if (numcpu[0][i] || numcpu[1][i] || numcpu[2][i] || numcpu[3][i])
		{
			...
			if (!ci_strcmp(cputype_name(i), "R3000 (big)"))
				sprintf(name, "%8s:  %3d  %3d  %3d  %3d\n", "R3000", numcpu[0][i], numcpu[1][i], numcpu[2][i], numcpu[3][i]);
			if (!ci_strcmp(cputype_name(i), "R4600 (little)"))
				sprintf(name, "%8s:  %3d  %3d  %3d  %3d\n", "R4600", numcpu[0][i], numcpu[1][i], numcpu[2][i], numcpu[3][i]);
			if (!ci_strcmp(cputype_name(i), "R5000 (little)"))
				sprintf(name, "%8s:  %3d  %3d  %3d  %3d\n", "R5000", numcpu[0][i], numcpu[1][i], numcpu[2][i], numcpu[3][i]);
			strcat(buffer, name);
	...
	for (i = 1; i < SOUND_COUNT; i++)
	{
		if (numsnd[0][i] || numsnd[1][i] || numsnd[2][i] || numsnd[3][i])
		{
			...
			if (!ci_strcmp(sndintf[i].name, "Namco 15xx"))
				sprintf(name, "%9s: %3d  %3d  %3d  %3d\n", "Namco 15", numsnd[0][i], numsnd[1][i], numsnd[2][i], numsnd[3][i]);
			if (!ci_strcmp(sndintf[i].name, "Namco 52xx"))
				sprintf(name, "%9s: %3d  %3d  %3d  %3d\n", "Namco 52", numsnd[0][i], numsnd[1][i], numsnd[2][i], numsnd[3][i]);
			if (!ci_strcmp(sndintf[i].name, "Namco 54xx"))
				sprintf(name, "%9s: %3d  %3d  %3d  %3d\n", "Namco 54", numsnd[0][i], numsnd[1][i], numsnd[2][i], numsnd[3][i]);
			if (!ci_strcmp(sndintf[i].name, "Sega 315-5560"))
				sprintf(name, "%9s: %3d  %3d  %3d  %3d\n", "Sega 315", numsnd[0][i], numsnd[1][i], numsnd[2][i], numsnd[3][i]);
			if (!ci_strcmp(sndintf[i].name, "YMF292-F SCSP"))
				sprintf(name, "%9s: %3d  %3d  %3d  %3d\n", "YMF292-F", numsnd[0][i], numsnd[1][i], numsnd[2][i], numsnd[3][i]);
	...
	for (n = 200; n < 400; n++)
	{
		...
		if (resx[n])
		{
			sprintf(name, " %4dx%d: %3d     ", resx[n], resy[n], resnum[n]);
			strcat(buffer, name);
		}

		if (palett[n-100])
		{
			sprintf(name, "%5d: %3d\n", palett[n-100], palettnum[n-100]);
			strcat(buffer, name);
		}
		else
			if (resx[n])	strcat(buffer, "\n");
