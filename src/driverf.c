...
TESTDRIVER( vshoot )	/* (c) 1994 */
	DRIVER( outfxies )	/* (c) 1994 */
	DRIVER( outfxesj )	/* (c) 1994 (Japan) */
TESTDRIVER( machbrkr )	/* (c) 1995 (Japan) */
...
TESTDRIVER( rckmanj )	/* 22/09/1995 Rockman: The Power Battle (Japan) */
	...
	DRIVER( gigawing )	/* 22/02/1999 (c) 1999 Giga Wing (US) */
	DRIVER( gwingj )	/* 23/02/1999 (c) 1999 Giga Wing (Japan) */
TESTDRIVER( jgokushi )	/* 27/05/1999 Jyangokushi: Haoh no Saihai (Japan) */
TESTDRIVER( dimahoo )	/* 21/01/2000 Dimahoo (c) 2000 (US) */
...
TESTDRIVER( progear )	/* 17/01/2001 Progear No Arashi */
TESTDRIVER( puzloop2 )	/* 05/02/2001 Puzz Loop 2 (Japan) */
TESTDRIVER( choko )		/* 20/08/2001 Choko (Japan) */
...
TESTDRIVER( dragngun )	/* MAR (c) 1992 Data East Corporation (US) */
	/* Wizard's Fire/Dark Seal 2   MAS (c) 1992 */
...
TESTDRIVER( tattass )	/*    (c) 1994 Data East Pinball */
	/* Backfire            MBZ (c) 1995 */
...
TESTDRIVER( rungun )	/* GX247 (c) 1993 (World) */
TESTDRIVER( rungunu )	/* GX247 (c) 1993 (US) */
TESTDRIVER( slmdunkj )	/* GX247 (c) 1993 (Japan) */
...
*/
	...
	/* Gaelco games */
	DRIVER( bigkarnk )	/* (c) 1991 */
	DRIVER( splash )	/* (c) 1992 */
TESTDRIVER( thoop2 )	/* (c) 1994 */
	DRIVER( biomtoy )	/* (c) 1995 */
TESTDRIVER( maniacsq )	/* (c) 1996 */
...
TESTDRIVER( sarukani )	/* (c) 1997 Kaneko (Japan) */
...
TESTDRIVER( dondenmj )	/* (c) 1986 Dyna Electronics */
TESTDRIVER( suzume )	/* (c) 1986 Dyna Electronics */
	DRIVER( tontonb )	/* (c) 1987 Dynax */
	DRIVER( mjdiplob )	/* (c) 1987 Dynax */
TESTDRIVER( majs101b )	/* (c) 1987 Dynax */

	/* Dynax games */
	DRIVER( sprtmtch )	/* (c) 1989 Log+Dynax + Fabtek license */
	DRIVER( drgpunch )	/* (c) 1989 Log+Dynax */
TESTDRIVER( ddenlovr )
TESTDRIVER( rongrong )
...
TESTDRIVER( mjkojink ) 	/* (c) 1989 Home Data */
	...
	DRIVER( mayumi )	/* (c) 1988 Victory L.L.C. */
TESTDRIVER( pirates )	/* (c) 1994 NIX */
...
TESTDRIVER( bbprot )
	...
	/* 0072 Karate Ninja Sho (prototype) 1995 Yumekobo */
	...
	/* 0204 Oshidashi Zintrick (CD only? not confirmed, MVS might exist) 1996 SNK/ADK */
	...
	/* 0210 Last Odyssey Pinball Fantasia (prototype) 1995 Monolith */
	/* 0211 QP (prototype) */
	...
	/* 0258 */
	DRIVER( bangbead )	/* 0259 (c) 2000 Visco (prototype) */
	DRIVER( nitd )		/* 0260 (c) 2000 Eleven / Gavaking */
TESTDRIVER( zupapa )	/* 0070 (c) 2001 SNK */
...
/* misfit games */
TESTDRIVER( vliner ) /* 0265 (c) Visco */ 
      DRIVER( diggermn )	/* 0066 (c) 1994 Face ? */
	...
	DRIVER( eltonpac )     /* Sil hack of Pac-Man (Midway) */
 	DRIVER( brakman )      /* Jerry Lawrence hack of Pac-Man (Midway) */
     	DRIVER( vectrpac )     /* T-Bone hack of Pac-Man (Midway) */
	DRIVER( roboman )      /* Rungedog hack of Pac-Man (Midway) */
	...
	DRIVER( mspac6m )      /* Sil hack of Ms. Pac-Man */
     	DRIVER( puckren )      /* Peter Storey hack of Puck-Man */
	...
	DRIVER( vecpengo )     /* T-Bone hack of Pengo */
 	DRIVER( xensrev )      /* Jeff Morris hack of Pac-Man */
	...
	DRIVER( sfzch )
	DRIVER( sfach )		/* 10/20/1995 HACK (CPS Changer) */
	DRIVER( vantris )
	DRIVER( mariopac )
	DRIVER( stickydo )
	DRIVER( gaunt2s )
	...
	DRIVER( jacman )
      ...
      // PacMAME games 23 Dec 2000
	// Commented out drivers are already defined above.
	/* PacMAME */

	//DRIVER( 4in1 )      /* Armenia 1981? */
	DRIVER( bucaneer )
	DRIVER( pnc )
	DRIVER( cookiem )
	DRIVER( baby2 )
