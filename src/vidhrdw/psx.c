/***************************************************************************
	...
	Thanks to Ryan Holtz, Pete B & Farfetch.
...
static int m_n_debugcoordx[ 20 ];
static int m_n_debugcoordy[ 20 ];
...
static int DebugTextureDisplay( struct mame_bitmap *bitmap )
{
	...
	if( m_b_debugtexture )
	{
		...
		for( n_y = 0; n_y < 1023; n_y++ )
		{
			draw_scanline16( bitmap, 0, n_y, 1023, m_p_p_vram[ n_y ], Machine->pens, -1 );
		}
	}
...
static int psx_gpu_init( int n_width, int n_height )
{
	...
	state_save_register_UINT32( "psx", 0, "m_n_drawarea_x2", &m_n_drawarea_y2, 1 );
...
VIDEO_UPDATE( psx )
{
	...
	verboselog( 2, "video_update start\n" );
#if defined( MAME_DEBUG )
...
static void FlatPolygon( int n_points )
{
	...
	for( n_point = 0; n_point < n_points; n_point++ )
	{
		SET_COORD_X( m_packet.FlatPolygon.vertex[ n_point ].n_coord, COORD_X( m_packet.FlatPolygon.vertex[ n_point ].n_coord ) + m_n_drawoffset_x );
		SET_COORD_Y( m_packet.FlatPolygon.vertex[ n_point ].n_coord, COORD_Y( m_packet.FlatPolygon.vertex[ n_point ].n_coord ) + m_n_drawoffset_y );
	}
	...
	for( ;; )
	{
		...
		if( (INT16)n_cx1.w.h != (INT16)n_cx2.w.h && n_y >= m_n_drawarea_y1 && n_y <= m_n_drawarea_y2 )
		{
			...
			{
				n_distance = ( (INT32)m_n_drawarea_x2 - n_x ) + 1;
			}
...
static void FlatTexturedPolygon( int n_points )
{
	...
	for( n_point = 0; n_point < n_points; n_point++ )
	{
		SET_COORD_X( m_packet.FlatTexturedPolygon.vertex[ n_point ].n_coord, COORD_X( m_packet.FlatTexturedPolygon.vertex[ n_point ].n_coord ) + m_n_drawoffset_x );
		SET_COORD_Y( m_packet.FlatTexturedPolygon.vertex[ n_point ].n_coord, COORD_Y( m_packet.FlatTexturedPolygon.vertex[ n_point ].n_coord ) + m_n_drawoffset_y );
	}
	...
	n_ty = ( ( m_packet.FlatTexturedPolygon.vertex[ 1 ].n_texture.w.h & 0x10 ) << 4 ) |
		( ( m_packet.FlatTexturedPolygon.vertex[ 1 ].n_texture.w.h & 0x800 ) >> 2 );
...
static void GouraudPolygon( int n_points )
{
	...
	for( n_point = 0; n_point < n_points; n_point++ )
	{
		SET_COORD_X( m_packet.GouraudPolygon.vertex[ n_point ].n_coord, COORD_X( m_packet.GouraudPolygon.vertex[ n_point ].n_coord ) + m_n_drawoffset_x );
		SET_COORD_Y( m_packet.GouraudPolygon.vertex[ n_point ].n_coord, COORD_Y( m_packet.GouraudPolygon.vertex[ n_point ].n_coord ) + m_n_drawoffset_y );
	}
...
static void GouraudTexturedPolygon( int n_points )
{
	...
	for( n_point = 0; n_point < n_points; n_point++ )
	{
		SET_COORD_X( m_packet.GouraudTexturedPolygon.vertex[ n_point ].n_coord, COORD_X( m_packet.GouraudTexturedPolygon.vertex[ n_point ].n_coord ) + m_n_drawoffset_x );
		SET_COORD_Y( m_packet.GouraudTexturedPolygon.vertex[ n_point ].n_coord, COORD_Y( m_packet.GouraudTexturedPolygon.vertex[ n_point ].n_coord ) + m_n_drawoffset_y );
	}
	...
	n_tp = ( m_packet.GouraudTexturedPolygon.vertex[ 1 ].n_texture.w.h & 0x180 ) >> 7;
	n_tx = ( m_packet.GouraudTexturedPolygon.vertex[ 1 ].n_texture.w.h & 0x0f ) << 6;
	n_ty = ( ( m_packet.GouraudTexturedPolygon.vertex[ 1 ].n_texture.w.h & 0x10 ) << 4 ) |
		( ( m_packet.GouraudTexturedPolygon.vertex[ 1 ].n_texture.w.h & 0x800 ) >> 2 );
	n_abr = ( m_packet.GouraudTexturedPolygon.vertex[ 1 ].n_texture.w.h & 0x60 ) >> 5;
...
static void MonochromeLine( void )
{
	...
	while( n_len > 0 )
	{
		/* todo: moving this outside the loop would be good */
		if( (INT16)n_x.w.h >= m_n_drawarea_x1 &&
			(INT16)n_y.w.h >= m_n_drawarea_y1 &&
			(INT16)n_x.w.h <= m_n_drawarea_x2 &&
			(INT16)n_y.w.h <= m_n_drawarea_y2 )
		{
...
static void FlatTexturedRectangle( void )
{
	...
	n_abr = ( m_packet.FlatTexturedRectangle.n_texture.w.h & 0x60 ) >> 5;
...
WRITE32_HANDLER( psx_gpu_w )
{
	...
	{
	case 0x00:
		verboselog( 2, "GPU Buffer Offset: %u\n", m_n_gpu_buffer_offset );
		...
		case 0x37:
			if( m_n_gpu_buffer_offset < 8 )
			{
				verboselog( 2, "textured gouraud polygon packet #%x\n", m_n_gpu_buffer_offset );
				m_n_gpu_buffer_offset++;
			}
			else
			{
				verboselog( 1, "textured gouraud polygon draw\n" );
		...
		case 0x3f:
			...
			{
				verboselog( 2, "textured gouraud quad packet #%x\n", m_n_gpu_buffer_offset );
				m_n_gpu_buffer_offset++;
			}
			else
			{
				verboselog( 1, "textured gouraud quad draw\n" );
		...
		case 0x67:
			...
			{
				verboselog( 1, "flat textured rectangle %d,%d %u,%u %08x, %08x\n",
					(INT16)( m_packet.n_entry[ 1 ] & 0xffff ), (INT16)( m_packet.n_entry[ 1 ] >> 16 ),
		...
		case 0x80:
			...
			if( m_n_gpu_buffer_offset < 3 )
			{
				verboselog( 2, "move image packet #%x\n", m_n_gpu_buffer_offset );
				m_n_gpu_buffer_offset++;
			}
			else
			{
				verboselog( 0, "not handled: move image %08x %08x %08x %08x\n", m_packet.n_entry[ 0 ], m_packet.n_entry[ 1 ], m_packet.n_entry[ 2 ], m_packet.n_entry[ 3 ] );
				m_n_gpu_buffer_offset = 0;
			}
			break;
		case 0xa0:
		...
		case 0xe1:
			...
			/* todo: work out where this bit is really stored */
			m_n_gpustatus = ( m_n_gpustatus & 0xffffdfff ) | ( ( m_packet.n_entry[ 0 ] & 0x800 ) << 2 );
			if( ( m_packet.n_entry[ 0 ] & 0xfff000 ) != 0 )
			{
				verboselog( 0, "unknown draw mode %08x\n", m_packet.n_entry[ 0 ] & 0xfff000 );
			}
			...
			{
				verboselog( 0, "not handled: draw to display area prohibited\n" );
			}
			break;
		case 0xe2:
