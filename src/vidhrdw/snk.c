...
PALETTE_INIT( snk_3bpp_shadow )
{
	...
	for(i = 0; i < 8; i++) gfx_drawmode_table[i] = DRAWMODE_SOURCE;
...
void tnk3_draw_text( struct mame_bitmap *bitmap, int bank, unsigned char *source )
{
	...
	for(x=0; x<32; x++) for(y=0; y<32; y++)
	{
		...
		drawgfx(bitmap,gfx,tile_number,color,0,0,sx,sy,0,TRANSPARENCY_PEN,15);
	}
}
...
void tnk3_draw_sprites( struct mame_bitmap *bitmap, int xscroll, int yscroll )
{
	...
	for(offs = 0; offs < 50*4; offs+=4)
	{
		...
		tile_number |= (attributes & 0x40) ? 256:0;
		tile_number |= (attributes & 0x20) ? 512:0;

		color = attributes & 0xf;
		sx =  xscroll - spriteram[offs+2] + ((attributes & 0x80) ? 0:256);
		sy = -yscroll + spriteram[offs]   + ((attributes & 0x10) ? 256:0);
		sx &= 0x1ff;
		sy &= 0x1ff;

		drawgfx(bitmap,gfx,tile_number,color,0,0,sx,sy,&clip,TRANSPARENCY_PEN,7);
	}
}
...
VIDEO_UPDATE( tnk3 )
{
	...
	*/
	...
	{
		int bg_scrollx = -ram[0xcc00] + ((attributes & 0x02) ? 256:0);
		int bg_scrolly = -ram[0xcb00] + ((attributes & 0x10) ? 256:0);
		bg_scrollx += 15;
		bg_scrolly += 8;
	...
	{
		int sp_scrollx = ram[0xca00] + ((attributes & 0x01) ? 256:0);
		int sp_scrolly = ram[0xc900] + ((attributes & 0x08) ? 256:0);
		sp_scrollx += 29;
		sp_scrolly += 9;
				...
				unsigned char *source, int mode )
{
	...
	for(which = start*4; which < (start+25)*4; which+=4)
	{
		...
		color = attributes & 0xf;
		sx =  xscroll - source[which+2] + ((attributes & 0x80) ? 0:256);
		sy = -yscroll + source[which]   + ((attributes & 0x10) ? 256:0);
		sx &= 0x1ff; sx += -16;
		sy &= 0x1ff; sy += -16;
...
static void tdfever_draw_sp( struct mame_bitmap *bitmap, int xscroll, int yscroll, int mode )
{
	...
	int tile_number, attributes, sx, sy;
	...
	for(which = 0; which < finish*4; which+=4)
	{
		tile_number = source[which+1];
		attributes  = source[which+3];
		...
		sx += (attributes & 0x80) ? 256:0;
		sy += (attributes & 0x10) ? 256:0;
		...
		switch(mode)
		{
			...
			case 1:
				tile_number |= (attributes & 0x60) << 3;
				drawgfx(bitmap,gfx,tile_number,attributes & 0xf,0,0,sx,sy,&clip,TRANSPARENCY_PEN,15);
				break;
			case 2:
				tile_number |= (attributes & 0x08) ? 0x100:0;
				tile_number |= (attributes & 0x20) ? 0x200:0;
				tile_number |= (attributes & 0x40) ? 0x400:0;
				drawgfx(bitmap,gfx,tile_number,attributes & 7,0,0,sx,sy,&clip,TRANSPARENCY_PEN,15);
				break;
		}
	}
}
...
static void tdfever_draw_tx( struct mame_bitmap *bitmap, int attributes, int dx, int dy, int base )
{
	...
	for(x = 0; x < 64; x++) for(y = 0; y < 32; y++)
	{
		tile_number = source[x*32+y] | tile_high;
...
VIDEO_UPDATE( tdfever )
{
	...
	if(!strcmp(Machine->gamedrv->name,"tdfever") || !strcmp(Machine->gamedrv->name,"tdfeverj"))
	{
			bg_scroll_x += 141;
			bg_scroll_y += -32;
			sp_scroll_x += 135;
	...
	if(!strcmp(Machine->gamedrv->name,"ftsoccer"))
	{
			bg_scroll_x += 16;
			bg_scroll_y += 0;
			sp_scroll_x += 40;
...
VIDEO_UPDATE( gwar )
{
	...
	if(!strcmp(Machine->gamedrv->name,"gwara"))
	{
